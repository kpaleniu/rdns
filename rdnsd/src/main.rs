...
use rdns::{Name, UdpSizes};
// Test-only since `TODO.md` #38d moved the transfer and UPDATE answering, which
// were the non-test callers, into `dispatch`. `notify` and `BTreeMap` joined
// them at #83, when the NOTIFY-out cluster left.
#[cfg(test)]
use rdns::{
    clock::current_unix_timestamp,
    notify::{self, NotifyPeer},
    tsig,
...
async fn reload_once(
    ...
    let (announced, outcome) = match reloading.load(source, previous).await {
        Ok(new_zones) => {
            ...
            (
                crate::notify_out::announce_zones(&served.zone_map, &announced, notify, busy).await,
                Ok(loaded),
            )
        }
        Err(e) => {
...
async fn main() -> Result<()> {
    ...
    let notify = Arc::new(notify_out::build_notify_policy(
        &cli.also_notify,
        &per_zone.notify,
        &tsig_keys,
    )?);
    ...
    let announced = notify_out::announce_zones(&zone_map, &[], &notify, &shutdown.busy()).await;
