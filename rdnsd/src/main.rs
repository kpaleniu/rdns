...
use rdns::{DnsMessage, ResponseCode, ResourceRecord, zone::{Zone, parse_zone_file}, 
           security::RateLimiter, validation::RequestValidator, logging::QueryLogger};
...
#[derive(Subcommand)]
enum Commands {
    Tcp { addr: String, #[arg(long)] zone_file: Option<String> },
    Udp { addr: String, #[arg(long)] zone_file: Option<String> },
}

/// Build a DNS response for the given query message
fn make_response(msg: &DnsMessage, zone: &Zone) -> DnsMessage {
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            if !validation.is_valid() {
                logger.log_error(
                    peer_addr.ip(),
                    &format!("invalid query: {}", validation.error_message().unwrap_or("unknown error")),
                );
                return;
            }
            ...
            if let Ok(msg) = DnsMessage::try_from_bytes(&buf[0..n]) {
                ...
                let resp = make_response(&msg, &zone);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.commands {
        ...
        None => {
            let zone = load_zone(&None)?;
            udp_main("0.0.0.0:53", Arc::new(zone)).await
        }
    }
}

/// Load a zone from a file, or create an empty example zone if no file is provided
fn load_zone(zone_file: &Option<String>) -> Result<Zone, Box<dyn std::error::Error>> {
    match zone_file {
        Some(path) => {
...
ns1 IN  A   192.0.2.10
            "#;
            let zone = parse_zone_file(example_zone, "example.com.")?;
            Ok(zone)
        }
    }
}
