...
- **#33** — a fourth review pass, filed 2026-09-05: duplication, generics, and
  where the modules are cut. Eight items, of which **33a, 33b and 33c are done**
  — 33b was the one defect in a shipped binary, `rdnsc` unable to ask an ANY
  query — and five are open, all consolidations. Five further candidates were
  dropped and the section says why, which is the half of it worth reading.
...
>
> **2. The rest of #33**, in the order its own "Order" section gives: **33d**,
> which 33f rides on, then **33e** and **33g**. All consolidations now; the
> defect (33b) and the measured item (33a) are done.
...
What is left is **33d**, which 33f rides on, then **33e** and **33g**. 33g is
unblocked by 33b, and what it inherits is `with_query` taking a `Qtype` rather
than the `pub mod testing` the first draft wanted. **33h** is worth doing when
that code is next opened and not before.
