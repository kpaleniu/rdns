...
| `TODO.md` #13e carries a claim the head commit falsified | `docs/CLOSED_WORK.md`, #13e (was `TODO.md:1466` before the 2026-09-05 move) | "≤255 octets … and *not* checked by `dname_to_bytes`, which validates label length and total length never". Commit `5506612` closed that: both now go through one `check_name_len` (`dname.rs:466`), and §15 records the fix (`docs/CLOSED_WORK.md`, #15; was `TODO.md:2033`). The stale sentence is in §13e's live body, not a preserved "as filed" block, so `CLAUDE.md` §11 applies — correct in place with a pointer to §15, keeping the reasoning |
