...
### `--catalog <ZONE@MASTER[:PORT][#KEY]>`

Consume a catalog zone (RFC 9432): a zone whose contents are the *list* of zones
this server should serve. Repeatable, spelled exactly like `--secondary`, and
requires `--zone-dir` for the same reason.

The catalog is itself replicated — it is an ordinary zone — so the flag implies
the `--secondary` spec for it, and everything under that flag applies to the
catalog: REFRESH and RETRY, NOTIFY, EXPIRE, the file on disk. What `--catalog`
adds is that after every transfer of it, the members it lists are brought into
line:

- a member the catalog lists and this server does not hold becomes a secondary
  of the *catalog's* master, signed with the catalog's key. RFC 9432 §7 is why
  the key is inherited rather than named in the catalog: "TSIG shared secrets
  used for member zones SHOULD NOT be mentioned in the catalog zone data";
- a member the catalog stops listing stops being served, and its zone file, its
  transfer state and its membership row are deleted (§5.3). Read RFC 9432 §6
  before pointing this at a catalog somebody else generates: "if the catalog is
  generated by some script and this script generates an empty catalog, millions
  of member zones may get deleted from their secondaries within seconds";
- a member whose node label (`<unique-N>`) changes is a removal followed by an
  immediate re-add, state and all (§5.4);
- a zone the configuration already names is never taken over, and neither is one
  another catalog provisioned: both are logged as errors and ignored (§5.2). A
  zone does move between catalogs when the one that holds it publishes a `coo`
  property naming the other (§4.3.1);
- a catalog that is broken — no `version.$CATZ` TXT of `"2"`, two PTRs at one
  member node, two nodes naming one zone — changes nothing at all. It is logged
  and the membership already in force stands (§5.1), and processing resumes by
  itself when the next transfer fixes it.

```bash
rdnsd --zone-dir ./zones \
  --catalog catalog.invalid.@192.0.2.1#transfer.key
```

In a config file it is a per-zone key, so the masters are written where every
other zone's are:
...
Two sidecars beside the zones, both plain text: `rdnsd.state` (see
`--secondary`) and `rdnsd.catalog`, one line per member — the zone, then the
member node it came from, which carries the catalog name. Deleting the second
one costs a removal nobody notices: a member dropped from the catalog while this
server is not running looks, at the next start, like a zone no catalog claims.

Members are not waited for by `/readyz`. It waits for the zones the
configuration names, the catalog among them; waiting for every member would keep
a cold-started consumer unready for as long as a producer's whole list takes to
transfer, which is the case this exists for.

`dns_catalog_members{catalog="..."}` on `--metrics-listen` is how many members
each catalog has this server serving. It is the number to alert on: RFC 9432 §6's
failure is that count falling off a cliff.
