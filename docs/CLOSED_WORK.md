...
Every numbered section that is finished, withdrawn or answered, and the log of
what landed. Moved out of `TODO.md` on 2026-09-05, when that file had grown to
7,989 lines of which about six thousand described work that was already done:
the queue was three screens and the archive was seventy, so the queue was the
part nobody could find.

**The prose is verbatim**, including the claims that turned out to be wrong and
the strike-throughs that correct them, because `CLAUDE.md` §11 keeps the
reasoning that produced a mistake rather than the tidy version. Two things
changed on the way across, and nothing else did. The order is numeric here,
where `TODO.md` had them in filing order; nothing was renumbered, because the
numbers are referenced from the code.

**#38, #39 and #40 came across on 2026-09-11**, having closed on 2026-09-10 and
2026-09-11 and then sat under "Open work" for a day or two — the same drift the
next paragraph records, caught this time by their rows in `TODO.md`'s table,
which said where the section still was until it was here. #38's heading was one
more instance of the fourteen: it said `filed` for work that had closed.

**And fourteen headings were corrected**, in §11's form — the original struck
through, the outcome after it. Eleven still said `filed` for work that had
closed (#7, #22, #24-#32) and three carried no status at all (#2, #14, #16). A
heading is the first thing read and was the last thing updated: #30's said
`filed 2026-09-04` while the section's own closing paragraph said every
remaining item landed on 2026-09-05, and its row in `TODO.md`'s table said
`open` on the same day. Three places, one status, two of them wrong.

**This is the page a code comment means.** 88 doc comments in the workspace
cite a section by number — `TODO.md` #30h, #27a, #25g, #26b, #28a and the rest.
They still say `TODO.md`, and they are still right to: that file carries one
line per closed item pointing here, so a reader following one of them lands on
the queue first and the reasoning one hop later. They were left alone on
purpose rather than rewritten to name this file, because an 88-comment
mechanical diff through the whole tree does not belong in an editorial change
(§12's rule about not mixing a reformat into a behaviour change, applied to
prose).

**What is *not* here:** open work, how to run any of it, the environment traps,
the architecture notes and the quick references. Those are `TODO.md`, which is
now about a fifth of what it was and is the file to read first. `docs/spec/`
says what the code does; this says what it took and why the shape is what it
is.

**Three sections have no prose of their own** and never did — #5, #8 and #9
were tracked as rows in `TODO.md`'s table. #9's 2,435 lines of finding text were
deleted on 2026-08-02 and are in `git log -p TODO.md`; what they became is
`CLAUDE.md`, which is the useful artefact. Their rows are in `TODO.md`'s closed
index with the rest.
...
### 7. The secondary role — ~~five steps done, one waiting~~ **all six done 2026-08-03**
...
### 14. What #13 left on the table — three candidates, not a plan — **all three done 2026-08-02**
...
### 16. Simplifications — a review pass, and what it did *not* find — **closed 2026-08-02**
...
### 24. Three costs that grow with something the operator chose — ~~filed 2026-08-04~~ **all three fixed 2026-08-05**
...
### 25. Per-answer waste on paths already measured — ~~filed 2026-08-04~~ **closed 2026-09-04**
...
### 26. Helpers written twice, and hand-rolls with a standard spelling — ~~filed 2026-08-04~~ **closed 2026-09-04**
...
| **26f** | ~~`resolver::normalize` allocates unconditionally~~ **fixed 2026-09-04, at the four sites that were lookups** | `resolver.rs:1779`; `utils::absolute_lowered` returns a `Cow` and borrows the common case. A straggler of #13b. **`normalize` stays** — it is the owning spelling, and ten of its fourteen callers keep the name as a map key or a set member, which is what its doc comment already said. The four that do not are lookups and take the `Cow` now: the delegation-cache read, its `forget`, the trust-anchor walk, and the bailiwick filter — the last of which runs **once per record of every response**, so its allocation was per record and not per query |
...
| **26h** | ~~`utils::record_type_name` returns `String` for a constant~~ **fixed 2026-09-04** | thirteen known mnemonics, each `"A".to_string()`. `&'static str` with a `Cow` for the `TYPEnnn` arm costs nothing and removes an allocation per record written. **Done as filed**, with the test asserting on the `Cow` variant rather than on the text — the text was already right, and the allocation is what changed |
| **26i** | ~~`dnssec_denial.rs:189` walks a bitmap byte bit by bit~~ **fixed 2026-09-04, with the filed call corrected** | ~~`trailing_zeros` is the idiom.~~ **`leading_zeros`**: bit 0 of a type bitmap is the *high* bit of the byte (RFC 4034 §4.1.2), so `trailing_zeros` would have listed each byte's types backwards and `bitmap_types` promises ascending. The note named the idiom without checking which end this bitmap starts at. Writer-side only, so smallest here; it lives in `denial_wire` now |
...
### 27. What a zero-allocation answer path would take — ~~filed 2026-09-01~~ **closed 2026-09-05, 27d withdrawn**
...
### 28. Work the answer path does and need not — ~~filed 2026-09-01~~ **28a-28c done and 28d answered no, 2026-09-01**
...
### 29. The resolver half never got #27's pass — ~~filed 2026-09-04~~ **closed 2026-09-04**
...
### 30. The two daemons' transports are one transport written twice — ~~filed 2026-09-04~~ **closed 2026-09-05**
...
| | what | note |
|---|---|---|
| **30a** | ~~the TCP transport, whole~~ **fixed 2026-09-05** | `rdnsd/src/main.rs:665-786` ↔ `rdnsr/src/main.rs:854-990`. Accept loop (semaphore, cancel-safe `accept` under `Stop`, spawn with permit and `Busy`) and connection loop (split, writer task owning the write half, `mpsc` of `MAX_INFLIGHT_PER_CONNECTION`, prefix read under `TCP_IDLE_TIMEOUT` with the stop check between messages, body under `TCP_READ_TIMEOUT`, drop-tx-then-join). 21 identical lines at `rdnsd:725-745` ↔ `rdnsr:925-942`; RFC 7766 §6.2.1.1, §6.2.3 and RFC 1035 §4.2.2 quoted in both. What is *not* shared is where each applies its admission policy — see 30e and 30q. **Done as `rdns_transport::tcp`**: `serve` (the accept loop), `serve_one` (one connection, which is also what a test wants), `Reply`, `send_framed`, and a `Handler` trait that is sink-shaped for the reason the note below gives. The admission policy is the `RateLimit` parameter, so each daemon's stays visible at its call site — provoked afterwards to prove neither moved: six pipelined messages on one connection get two answers from `rdnsd` at a burst of two and six from `rdnsr`, which is per message against per connection |
| **30b** | ~~five transport constants~~ **fixed 2026-09-05** | `rdnsd:53-70` ↔ `rdnsr:34-51`. 10 s, 5 s, 128 and 16 identical, 4096 under two names, each reason reworded rather than shared. **Not a shared `const`, though**: one constant forces an authoritative server and a resolver to hold the same connection ceiling forever, and they have no reason to. Defaults on a policy struct, which is where 30k lands too. **Done as `TransportLimits`** in `rdns-transport`, with `Default` carrying the four both daemons ran with. The fifth, the advertised EDNS payload size, stays per daemon: it is what each tells a client it can receive, not what a connection may cost |
| **30c** | ~~`listener_failure`~~ **fixed 2026-09-05** | `rdnsd:653` ↔ `rdnsr:537`, identical but for `anyhow!` against `anyhow::anyhow!`. **It cannot move into `rdns`**: it returns `anyhow::Error`, the library has no `anyhow` dependency and §3 says it must not grow one. It wants `rdns-transport` — which may use `anyhow`, its only consumers being the two daemons — so 30c is an argument for #31's shape rather than a thing to do before it. **And it was the argument that built the crate**: `rdns-transport` may use `anyhow`, and this function is the smallest thing that proves the boundary is about a dependency rather than about tidiness |
| **30d** | ~~the shutdown epilogue~~ **fixed 2026-09-05, and it was hiding a defect** | `rdnsd:622-659` ↔ `rdnsr:508-543`. `JoinSet` + `select!{join_next, stop_signal}` + `begin()` + drain the set + `drain_reporting()` + `match failure`, including the comment saying why it is not two `JoinHandle`s. Same `anyhow` problem as 30c, same conclusion. **Done as `serve_until_stopped`**, which also takes the one task that is *not* a listener — the anomaly watcher — because a `JoinSet` whose rule is "the first task to end ends the process" must not hold a watcher that returns at once on `--anomaly-interval 0`. **The defect**: `rdnsr` took `(shutdown.stop_handle(), shutdown.busy())` *before* the `if let Some(metrics_listener)` that consumes them, so with no `--metrics-listen` — the default — `main` held a `Busy` for the life of the process. Every shutdown then waited out the whole drain budget and warned that work was still running when none was: `shutdown drain hit its budget with work still running; exiting anyway budget_secs=5` at 5 s, against `drained cleanly` and an immediate exit now. Pre-existing, and found only because the epilogue moving made it worth watching a process actually stop (`CLAUDE.md` §4) |
| **30e** | ~~the UDP admission pipeline, twice~~ ~~**the admission pipeline, four times and no two alike**~~ **fixed 2026-09-05** | ~~`rdnsd:1665-1730` ↔ `rdnsr:752-850`, the same sequence with one drift.~~ **Wrong, corrected on review.** The UDP halves do match (limiter → `validate_packet` → answer → `ResponseVerdict`), but the four paths differ in three ways that a shared pipeline has to be told about rather than smooth over: `rdnsd` splits the sequence across `udp_loop` and `answer_datagram` for UDP and writes it whole in `answer` for TCP, so it is already twice inside one binary; `rdnsd` rate-limits per TCP **message** (`:805`) while `rdnsr` rate-limits per TCP **accept** (`:872`) and neither does both; and `rdnsr` logs no validation failure where `rdnsd` does. The clock read differs too, on purpose — see "what must not be unified". **Done as two functions rather than one pipeline**: `ServeContext::allow_source(peer, now)` and `accept_packet(peer, packet, transport)`, composed at each of the four sites. That keeps the two differences visible where they are decided — `rdnsr` still charges the limiter per *accept* on TCP and `rdnsd` per message — instead of a shared pipeline having to be told about them. `now` is a parameter for the reason below. The sharing gave `rdnsr` the validation-failure logging and the error count it did not have |
| **30f** | ~~the response budget derived by hand, twice~~ **fixed 2026-09-04** | `rdnsd:516-520` ↔ `rdnsr:415-419`, both `ResponseLimiter::new(rate, rate.saturating_mul(4), 2)`. The `×4` ("four seconds' worth") and the `2` ("BIND's own slip default") are written on `security.rs:255`'s `with_defaults()` — **which no binary calls**; it is reached only by its own test. And the `if rate == 0 { disabled() }` arm both binaries write is **provably dead**: `disabled()` *is* `new(0, 0, 0)`, `is_enabled` is `bytes_per_sec > 0`, and `admit` returns `Send` when it is false — so `new(0, …)` already disables. One `per_second(rate)` replaces the branch and the arithmetic, and `with_defaults()` is now `per_second(8192)` — the same number both `--response-rate` flags default to. A test asserts the dead arm was dead |
| **30g** | ~~five hand-written reply skeletons~~ **fixed 2026-09-04, both halves** | `rdnsd:1435` (`error_bytes`), `rdnsd:1799` (the TSIG rejection — a copy of `error_bytes` from 360 lines up the same file, differing only in the length ceiling), `rdnsd:1566` (`truncated_reply`), `rdnsr:1401` (`unsupported_opcode`), `rdnsr:1429` (`build_response`), plus two in `notify.rs`. **Two items, not one.** ~~`rdnsd:1799` is an in-file copy and a straight fix.~~ **Done**: `error_bytes` takes the ceiling as an argument, which was the whole difference — `u16::MAX` on TCP, the client's EDNS payload size on UDP. The library half is a design question: `DnsMessageBuilder` builds *queries* only, so what is missing is a constructor that copies the echo fields — id, opcode, question, CD — and leaves the policy fields (AA, RA, AD) to the caller, since those are exactly where the five disagree. A function returning a finished message would fit none of them. **Done as `DnsMessage::reply_to`** — id, opcode, RD, question and CD copied, AA/RA/AD/RCODE/OPT left neutral — and it turned up a defect the duplication was hiding: `rdnsr`'s `build_response` **cleared CD**, against RFC 4035 §3.2.2's "The name server side MUST copy the setting of the CD bit from a query to the corresponding response" (fetched and quoted, not recalled). Two of its eight call sites set it back by hand; the other six answered a CD client with CD off. Six sites in `rdns`, `rdnsd` and `rdnsr` now share the constructor |
| **30h** | ~~the EDNS pre-checks and the mirroring, twice~~ **fixed 2026-09-04** | `answer.rs:52-67` + `:115-121` ↔ `rdnsr:1036-1053` + `:1358-1370`. `edns_header()` → FORMERR, a version past `EDNS_VERSION` → BADVERS, then an OPT only if the client used EDNS with DO mirrored. Same rules, same RFC 6891 §6.1.1 and §6.1.3 citations. **What is shared is a decision, not a writer**: `rdnsd` sets it on a `ResponseWriter` and `rdnsr` on a `DnsMessage`, so the moveable part is a pure function of `&DnsMessage` returning reject-with-this-rcode or accept-with-this-DO. `rdnsr`'s `finish` also *strips* DNSSEC records without DO — that is resolver-only, filtering what an upstream sent, and is not part of this. **Done as `response::client_edns` → `Result<ClientEdns, ResponseCode>`**, exactly the pure function the filing described, plus `ClientEdns::mirror(payload_size)`, which is the decision the two writers share; the payload size stays each daemon's. `rdnsr::finish` lost a second bool parameter to it |
| **30i** | ~~two NOTIMP implementations~~ **closed as a consequence 2026-09-04, with one difference recorded** | `answer.rs:72-88` inside `write_response`, `rdnsr:1400` as `unsupported_opcode`. **Falls out of 30g and 30h** — echo the opcode, echo the question, mirror the OPT — and is not independent work. **It did**: both now build on the shared skeleton and neither was touched on its own. What is left is an *ordering* difference, and it is deliberate on neither side — `rdnsd` classifies EDNS before it looks at the opcode and `rdnsr` after, so a NOTIFY carrying a malformed option list is FORMERR from one and NOTIMP from the other. Both are defensible and RFC 6891 orders neither; recorded here because it is the kind of thing a later reader would call a bug in whichever daemon they read second |
| **30j** | ~~two truncate-on-budget answers~~ **answered 2026-09-04: they stay two** | `rdnsd:1565` rebuilds from the *request* and bounds by `request.udp_payload_size()`; `rdnsr:733` re-parses its own *reply* and bounds by `CLASSIC_UDP_SIZE`. Both are safe — an empty TC=1 reply cannot exceed 512 even with a maximal question — so this is a consolidation, not a defect, and it **falls out of 30g** rather than being worth its own change. **It did not, and the reason is the answer**: `rdnsd` truncates where it still holds the request, and `rdnsr` truncates in the task that has already serialized its reply — the request's bytes were moved into `handle_query` and are gone. A shared skeleton needs a request; the resolver has a reply. `rdnsd`'s half is on `reply_to` now and `rdnsr`'s re-parse stays, on purpose |
...
| **30l** | ~~`restrict_to_owner` twice~~ **fixed 2026-09-04** | `rdnsd/src/control.rs:98` ↔ `rdns/src/persist.rs:99`, identical `set_permissions(0o600)`. ~~Export `persist`'s.~~ **Exported**, with the reason in its doc comment (§7): the mode goes on before the path is published, since a chmod after the rename leaves a window at the umask. **Only the one-liner**: `control::bind` is bind-temp → chmod → rename and `persist::write_atomically` is create-temp → write → fsync → chmod → rename, deliberately the same shape for the same reason — the mode is in place before the published path exists — but the middles are a socket and a file. There is no chmod-after-publish race in either |
| **30m** | ~~`rdnsr` holds the logger and does not call it~~ ~~**nothing reads the logger's aggregation, in either binary**~~ **the reader was written instead, 2026-09-04** | ~~A straggler of #18: wire `log_query` and `count_error` into `rdnsr`.~~ **Backwards, corrected on review.** `QueryLogger::get_stats` and `check_anomalies` have **no production caller anywhere** — the only non-test reference in the workspace is an `rdnsd` test. So `QueryStats` (total_queries, total_errors, qps, `queries_by_ip`, `queries_by_type`, `rate_limited_ips`) is write-only: `rdnsd` pays a mutex and two hash inserts per query for it, on the path #28 counts mutexes on, and `queries_by_type` duplicates the per-QTYPE atomics `DnsMetrics` already exports. `rdnsr` not calling it is evidence for deleting the aggregation, not for wiring it in. `log_rate_limited` stays: its map is the one §5 bounds. **Resolved the other way, after asking what the thing was for.** The commit that introduced it (`3a6110d`) says: "`logging` keeps per-IP and per-type counts and **warns on the shapes that precede a flood**", against "`metrics` is atomic counters rendered in the Prometheus text format" in the same list — so it was drafted as periodic *log* warnings, never as a scrape or an `rdnsctl` command, and the timer was simply never written. `git grep check_anomalies` over every commit finds no caller in either binary, ever. What it holds that `DnsMetrics` cannot is the *address*: per-source cardinality is exactly what a Prometheus label must not carry, and "which client is flooding us" is the question the counters answer. So: `watch_anomalies` on a timer, thresholds as flags on both daemons (§14 — the four were hardcoded at 50 / 10% / 100 / 5, each now its own off switch), and `rdnsr` fills the counters it held empty. The counts are per interval now, which they had to become before a timer could read them: cumulative counts would have warned about a client every minute forever after its one busy minute |
| **30n** | ~~one unused dependency, one misplaced~~ **fixed 2026-09-04** | `rdnsr/Cargo.toml` declares `tracing-subscriber` and never names it; `rdnsd`'s only use is in a test (`main.rs:2826`, `:2850`), so it belongs under `[dev-dependencies]`. Both done; `Cargo.lock` loses one edge and no package, because `rdns::logging` still installs the subscriber |
| **30o** | ~~`rdnsc` reimplements the resolver's exchange~~ **the predicate moved 2026-09-04; the socket dance stays two** | `rdnsc/src/main.rs:84-179` against `resolver.rs:746-760` and `:1191-1281`: family-matched bind, `connect` for the cheap half of off-path resistance, retry, TC→TCP fallback, id-and-question match, RFC 5452 §9.1 spelled out in both. **What can move is the predicate, and only it.** `matches_request` / `response_matches` is a pure function of two messages; the socket dance cannot be shared without making `rdnsc` async, which is the opposite of #31. `rdnsc`'s copy also has no DNS-0x20, which is a gap in the client rather than in the sharing. **Done as `validation::answers_query` + `SentQuery`**, beside `Request` because it is the same kind of door from the other side — and the library's copy was the weaker one: it compared the id and the name and **not the type or class**, which RFC 5452 §9.1 lists as a MUST ("Query class and type", fetched and quoted), and did not check QR either. `OutgoingQuery` now carries the qtype and qclass it sent. Watched failing: an MX-questioned reply was taken as the answer to an A query |
| **30p** | ~~the family-matched bind, three times~~ **fixed 2026-09-04** | `rdnsc:86-90`, `resolver.rs:1199-1204`, `rdnsd:2334-2338`. Pure, no I/O, no `tokio`: a `fn bind_addr_for(target) -> SocketAddr` in `utils` serves all three, sync and async alike. **Done as filed**, with a test for the v4-mapped v6 case, which is a v6 peer and had no copy that said so |
| **30q** | ~~**`rdnsr` never applies the admission check to TCP**~~ **fixed 2026-09-04** | `validate_packet` was called once in `rdnsr`, at `:795`, with `is_tcp = false`. The TCP path goes `serve_connection` → `handle_query` → `Request::from_bytes` with nothing in between, so on that transport none of `AdmissionCheck`'s pre-parse caps apply: the 16 KiB TCP ceiling (`validation.rs:62`, chosen deliberately and commented "not a protocol limit"), the 12-byte floor, QDCOUNT ≤ 10, "a QUERY carries no answer records", the per-section caps. `rdnsd` calls it on both transports (`:811` with `true`, `:1706` with `false`). Bounded — 128 connections × 16 in flight × 64 KiB, and the peer completed a handshake — so it is a resource and conformance gap, not a remote kill switch. **The shape is the one §7 exists for**: the facility is built, tested, configured into `rdnsr`'s `Shell` and applied on one of two paths. #18's own note said the risk was that a resolver's *operational* shell would be `rdnsd`-shaped; this is that, one transport down. **The fix is the one call, before the spawn** (§9: admission is decided before a task exists) and the message is skipped rather than the connection, which is what `rdnsd` does. The regression test was watched failing against the old code — the over-cap message came back answered |
| **30r** | ~~`rdnsr` answers a QUERY with two questions~~ **fixed 2026-09-04, filed the same day** | Found doing 30g rather than by the survey: RFC 9619 §4 says a message with OPCODE = 0 and QDCOUNT > 1 "MUST be treated as an incorrectly formatted message", `rdnsd` has refused it since #9f, and `rdnsr` answered the *first* question while echoing one — so the reply did not match the request either. It surfaced because the shared skeleton echoes the request's questions, which is right and is wrong for a resolver that answers only one of them. FORMERR now, with the same citation `answer.rs` carries |
...
watches it fail.~~ **Done 2026-09-04**, and it was one call and one test.
~~Then 30f, 30l, 30n and the `rdnsd:1799` half of 30g: a few lines each,
independent, no design question in any of them.~~ **Those four went the same
day**, and 30f's dead branch was dead as filed. **30m went the same day too**,
and not by deletion: the origin commit says the aggregation was drafted as
periodic log warnings, so the missing piece was the timer. ~~30h and the library half of
30g belong in `rdns` beside `response.rs`; they are protocol, not transport.~~
**30g's library half went the same day too** — `DnsMessage::reply_to` in
`rdns`, which is where it belonged — and brought two conformance fixes with it
(the cleared CD, and 30r). **30h followed**, as `response::client_edns`, and
30i and 30j fell out of the pair as filed: 30i closed with an ordering
difference recorded, 30j answered "they stay two" with the reason. **Then 30p
and 30o**, the `rdnsc` pair — 30o's predicate found the library holding the
weaker copy, missing RFC 5452 §9.1's type and class compare.
...
all.~~ **All five done 2026-09-05**, in two commits, once the crate existed. 30k is recorded and not to be done. 30i and 30j are consequences, not work. The naming
...
### 31. Where a crate boundary would pay — ~~filed 2026-09-04~~ **done 2026-09-05**
...
**`rdns-transport` exists as of 2026-09-05**, holding what needs no handler
trait: `ServeContext` (#32), `TransportLimits` (30b), `listener_failure` (30c)
and the admission pair `allow_source`/`accept_packet` (30e). ~~30a, the TCP
transport itself, and 30d, the shutdown epilogue, are the second stage.~~ **Both
went the same day**, so 30a-30e are closed and what is left of #30 is the
library half of 30g — already done — and nothing else. The handler is a trait
and the loop is not, which is the shape the "what must not be unified" note
asked for.

~~**The three-way cut of `rdns` itself is still open**, and unblocked: #26b moved
the wire helpers out of `dnssec_denial` on 2026-09-04, which was its one
prerequisite.~~ **Cut on 2026-09-05, and as two crates rather than three.**

`rdns-core` is the wire format and the names in it — the root types,
`dname`, `record_data`, `compression`, `response`, `utils`, `validation`,
`control`, most of `error`, and `build.rs`'s version stamp. No `tokio`, no
`ring`, no `sha`. `rdns` keeps everything above the wire and re-exports core
whole, so `rdns::DnsMessage` and `rdns::utils` still name what they always did
and the daemons changed by one line each.

Measured the same way before and after — unique `name version` lines from
`cargo tree -e normal`:
...
**The floor is clap, as filed**: 21 of the client's remaining 35. What is left
beside it is `rand` (the transaction id must not be predictable), `base64`,
`thiserror` and `anyhow`.

**Three claims of the plan needed correcting, and one of them is the shape of
the answer:**

- **Two crates, not three.** Splitting DNSSEC from net buys nothing measurable:
  neither client takes either, so the saving is the same, and a split without a
  measurement is motion. The three-way version stays available for the day a
  consumer wants DNSSEC without `tokio` — the module graph admits it, which is
  what walking the edges established.
- **`error` split as predicted, and that is what fixed the crate's shape.**
  `impl From<tokio::time::error::Elapsed>` can only be written where the error
  type is, so `ResolveError` and `TransferError` stayed on the net side with
  the impls; `rdns::error` re-exports core's, so §3's "all in `rdns::error`"
  still holds for a daemon reading one import path.
- **Two `pub(crate)` items had to become `pub`**: `dname::MAX_NAME_LEN` and
  `dname_to_bytes_in`, which the NSEC3 closest-encloser walk needs. A crate
  boundary is where `pub(crate)` stops meaning "internal".

Two tests crossed the boundary and neither was lost: the DNSSEC validator's
integration test moved to `dnssec_validation_mode`, and `utils`'s
"a suffix is a slice" test kept the half about slicing while the half asserting
the two spellings *agree* moved to `dnssec`, which is the crate that can name
both.

---

### 32. `Shell`, `Served`, and the other unnamed bags — ~~filed 2026-09-04~~ **closed 2026-09-05**
...
- [x] `ServeContext` in `rdns-transport`: those five and nothing else. Waits on
      the crate: without one, "shared by both binaries" means a copy in each,
      which is the duplication this item exists to name. **Done 2026-09-05**,
      with the crate.
- [x] `rdnsr`'s `Shell` becomes it, `record_answer` included. **Done
      2026-09-05** — and `record_answer` turned out to be a hand-rolled copy of
      `DnsMetrics::count_response`, arm for arm, so the shared one is that call
      plus the latency sample.
- [x] `rdnsd`'s `Server` holds one in place of the five loose fields — and keeps
      its own name. **Done 2026-09-05.** `answer`, `answer_transfer`, `answer_update`,
...
- [x] `Served` → `ZoneContext`, `Replication` → `ReplicationContext`. Both are
      ...
      **Done 2026-09-04**, ahead of the other three because these two need no
      crate to move into. `Server::served()` is `zone_context()` with them; the
      *field* and parameter spellings stay `served`, which reads as English
      ("what is served") where the type did not.
...
**Closed 2026-09-05, and the prediction held**: naming the five was the
extraction. `ServeContext` went into `rdns-transport` as 30e's parameter list
in the same commit that moved the admission pipeline, because there was nothing
else it could be — a bag named for what it is, holding what two daemons had
spelled two ways.
...
### 38. A structural review, and what it left — ~~**filed 2026-09-09**~~ **closed 2026-09-10**
...
- **The last three of #26** (26f, 26h, 26i) — the resolver's four *lookup*
  sites take `absolute_lowered`'s `Cow` instead of an owned copy, one of them
  per record of every response; `record_type_name` returns `Cow<'static, str>`,
  so writing a zone stops building thirteen names that are in the binary
  already; and the type-bitmap walk takes set bits with `leading_zeros` rather
  than testing all eight. The filed note said `trailing_zeros`, which is the
  other bit order and would have reversed the output.
...
- **The anomaly warnings have a caller, and their thresholds have flags**
  ...
  runs it every `--anomaly-interval` (default 60s, 0 off) on both daemons, and
  the four numbers that were hardcoded are `--anomaly-query-rate`,
  `--anomaly-error-percent`, `--anomaly-source-queries` and
  `--anomaly-source-refusals`, each with 0 as its own off switch, on the flags
  and in `rdnsd`'s config file. `take_stats` makes the counts per interval,
  which is what a threshold can be stated against; `rdnsr` now fills the
  counters it had been holding empty. What this keeps that `DnsMetrics` cannot
  is the source address — unbounded label cardinality is a metric's problem and
  a log line's whole point.
- **One answer-matching predicate, and the library had the weaker copy**
  (#30o, #30p) — `validation::answers_query` checks what RFC 5452 §9.1 makes a
  MUST and the socket does not: the id, the name, and the class and type. The
  resolver compared the id and the name only, and did not check QR, so a reply
  echoing another type was this query's answer. `utils::bind_addr_for` is the
  family-matched wildcard bind the same three call sites wrote out.
- **One EDNS decision instead of two** (#30h) — `response::client_edns` reads a
  request's OPT and returns either the RCODE that refuses it (FORMERR for an
  option list that does not parse, BADVERS past `EDNS_VERSION`) or a
  `ClientEdns` saying whether to attach an OPT and what to set DO to.
  `ClientEdns::mirror(payload_size)` is the reply half, so the size each daemon
  advertises stays its own. 30i and 30j resolved with it.
- **One reply skeleton instead of six** (#30g, #30r) — `DnsMessage::reply_to`
  copies what a reply must echo (id, opcode, RD, question, CD) and leaves AA,
  RA, AD, the RCODE and the OPT to the caller, which is where the six copies
  disagreed. Two defects fell out of writing it down: `rdnsr` cleared CD on
  every answer bar two, against RFC 4035 §3.2.2, and answered a two-question
  QUERY that RFC 9619 §4 says is malformed.
- **#30's one-line duplicates** (30f, 30g, 30l, 30n) — the response budget's
  four-seconds-of-burst and BIND slip of 2 were derived by hand in both daemons,
  around a `rate == 0` branch that was already dead, and are
  `ResponseLimiter::per_second`; `rdnsd`'s UDP TSIG rejection was `error_bytes`
  from 360 lines up the same file with a different length ceiling, which is now
  that function's argument; `restrict_to_owner` was written twice, and
  `rdns::persist`'s is the copy that carries the reason; `rdnsr` declared
  `tracing-subscriber` and never named it, and `rdnsd` names it only in tests.
- **`rdnsr`'s TCP transport had no admission check** (#30q) — `validate_packet`
  ran on the UDP loop and nowhere else, so on TCP none of `AdmissionCheck`'s
  pre-parse caps applied: the 16 KiB ceiling, the 12-byte floor, QDCOUNT ≤ 10,
  the per-section counts. Bounded rather than fatal — the peer completed a
  handshake and the connection ceilings still held — so a resource and
  conformance gap. The shape is §7's: the facility was built, tested and
  configured into `rdnsr`'s `Shell`, and applied on one of two paths.
...
## How the queue kept going stale, kept verbatim

`TODO.md`'s preamble and its open-work table are below as they stood on
2026-09-05, immediately before the rewrite that moved everything above into this
file. They are here rather than deleted for the reason they were written that
way in the first place: **a count in a preamble is a claim that goes stale
whenever the list below it changes**, and this is nine instances of it happening
to the same three paragraphs, each corrected in place rather than reworded
(`CLAUDE.md` §11). The strike-throughs are the record.

What the rewrite did about it is not a better paragraph. It is a shorter list:
the queue in `TODO.md` is now one open section and one inventory, and everything
finished is a row in a table beside a pointer here. A summary of two items
cannot drift from them. Whether that holds is a question for the next reader who
finds this page wrong.

### The preamble as it stood

#### `# rdns — TODO / Next Steps` — the title and preamble
...
**A fourth pass, 2026-09-05, asked #30's question of the whole tree** rather
than of the transports, and filed **#33**: is anything written twice, would a
generic or a trait collapse it, and are the modules and crates cut in the right
places. The last of those three is the one that produced almost nothing, and
**five candidates were dropped — four by rules already on this page**, which is
the half worth reading; a `dnssec/` module directory died on #31's own rule that
a split without a measurement is motion. The one defect is **33b**, and it is in
a shipped binary: `DnsMessageBuilder` resolves its question type through an
**RTYPE** table, so ANY, AXFR and IXFR find nothing there and the question is
dropped in silence — `rdnsc` cannot ask an ANY query at all, which is why
nothing here exercises the RFC 8482 path #9f fixed in `rdnsd`. **33a is the one
with teeth**: `NegativeCache` kept the per-insert `min_by_key` scan that
`DnsCache` was fixed away from, measured at 14.6 µs against 0.26 at the same
bound, on the mutex every query takes. Two of this filing's own claims died on
review — a `zone_writer` bug that is not reachable, and a trait that is the
wrong shape for what is actually duplicated — and both are struck through in
place.
...
### The open-work table and "where to pick up next", as they stood

#### `## Open work` — the preamble and the table
...
### 45. What an ISP would find missing in `rdnsr` — ~~**filed 2026-09-11**~~ **closed 2026-09-13**
