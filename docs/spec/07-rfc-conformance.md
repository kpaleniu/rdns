...
| 9432 | catalog zones | **the consumer side** (2026-09-12): §4's whole schema incl. `coo` and `group`, §5.1's broken-catalog rule, §5.2's clash rule, §5.3's removal, §5.4's state reset, §4.3.1's migration, and §4.3.2's group values mapped onto a per-catalog `groups` table (2026-09-13, `TODO.md` #48). Nothing on the control socket reports catalog membership, which §6 advises (`TODO.md` #49). The producer side needs no code — a catalog is an ordinary zone | `rdns/src/catalog.rs`, `rdnsd/src/catalog.rs` |
